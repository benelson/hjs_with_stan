...
real lnlike( real theta, real gamma, real xlower, real xupper ) {

    for (k in 1:2) { //loop over mixture components
    	if (n==1)
        {
...
transformed parameters{
    ...
    for (i in 1:Ni)
    {
        ...
        xi[i] <- 0.462 * (peri_true[i]/365.242)^(0.66667) * (Mpli_true[i] * MJtoMsun)^(0.3333) / ( radi_true[i] * RJtoAU );
    }
}
model {
    real ps[K];
