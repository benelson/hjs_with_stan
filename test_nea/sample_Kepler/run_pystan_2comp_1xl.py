...
modelfile = "powerlaw_2comp_1xl.stan"
nChains = 10
nIterations = 8000000
nThin = 4000
nJobs = -1
...
Stan_ICs = generate.ICs_2comp_1xl([ [0.5,0.5], [1.,1.], 1.5, 500. ], Stan_data["x"], nChains)

fit = pystan.stan(file=modelfile, data=Stan_data, iter=nIterations, chains=nChains, thin=nThin, init=Stan_ICs, n_jobs=nJobs)

postSamp = getData.stan_output_to_posterior_samples_2comp_1xl(fit.extract())

print(postSamp)

with open("postSamp_Kepler_2comp_1xl.txt", "w") as f:
